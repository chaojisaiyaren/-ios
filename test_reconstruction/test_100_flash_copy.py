...
from app.student.homework.object_page.games_page import Homework, FlashCard,Flash_copy
    ...
    def flash_exersise_stduy(self):
        ...
        for i in range(0, len(count)):
            ...
            if var == ' 闪卡练习 ' :
                ...
                self.copy_page.copy_type_study_reconstruction()
                print("进入抄写模式成功")

                # time.sleep(1)
                # self.two_homework.chick_question()
                time.sleep(1)
                self.copy_page.copy_flash_reconstruction()
                self.copy_page.mark_again()
                self.copy_page.copy_flash_reconstruction1()
                time.sleep(1)
                self.copy_page.return_btn()
                time.sleep(1)
                self.copy_page.return_btn()
                time.sleep(2)
