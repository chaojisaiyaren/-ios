...
from utils.vob_data import vob_operate
...
class Games(unittest.TestCase):
    ...
    def vob_selection_explantion(self):
        ...
        for i in range(0, len(count)):
            ...
            if var == ' 词汇选择 ' and title_type == '选解释':
                self.vob_page.vob_reconstrction()
                print("进入题目，开始答题")
                self.mothed()
                self.vob_page.end_page()
                self.vob_page.again_problem()
                self.mothed()
                self.vob_page.end_page()
    ...
    def mothed(self):
        ...
        for i in range(int(max_num)):
            ...
            ex = self.vob_page.options_word_ex(self.vob_option())
            print("我们选择正确的单词解释是：",ex)
    ...
    def vob_option(self):
        # 得到我们正确答案的列表的下表盘
        time.sleep(2)
        value = vob_operate(self.vob_page.word())
        time.sleep(2)
        print(value)
